...
//     const placeOrderInBackend = async (product) => {
//     try {
//       await axios.post("http://localhost:5000/api/user/order", {
//         userId: userInfo.email, // or however you uniquely identify users
//         productId: product.id
//       });
//     } catch (error) {
//       console.error("Error placing order in backend:", error);
//     }
//   };
...
//       await saveOrderToFirebase([...orders, productOrderDetails]);
//       await placeOrderInBackend(product); // <-- Add this line
...
//         await placeOrderInBackend(cartItem); // send each cart item to backend
//       }
...
//   try {
//     await setDoc(OrdersRef, { orders: sanitizedOrder }, { merge: true });
//   } catch (error) {
//     console.error('Error saving orders to Firebase:', error);
//   }
// };
...
const OrderSummary = () => {
  ...
  let productQty = product?.quantity || 0;
  let productPrice = product?.price || 0;
  let productTotalPrice = productPrice * productQty;

  let deliveryCharges = (productTotalPrice || cartTotalPrice) < 499 ? 40 : 0;
  ...
  const placeOrderInBackend = async (product) => {
    if (!product?.id || !userInfo?.email) {
      console.error("Missing product.id or userInfo.email", product?.id, userInfo?.email);
      return;
    }
    ...
    try {
      ...
      console.error("Error placing order in backend:", error.response?.data || error.message);
    }
  };
  ...
  const makePayment = async () => {
    ...
    if (product) {
      ...
      dispatch(addToOrders([...orders, productOrderDetails]));
      updateUserOrders([...orders, productOrderDetails]);
      await saveOrderToFirebase(productOrderDetails);
      await placeOrderInBackend(product);
      ...
      for (const item of updatedCart) {
        await saveOrderToFirebase(item);
        await placeOrderInBackend(item);
      }
